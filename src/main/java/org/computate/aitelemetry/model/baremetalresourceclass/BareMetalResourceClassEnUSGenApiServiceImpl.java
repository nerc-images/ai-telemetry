...
import org.computate.aitelemetry.model.baremetalresourceclass.BareMetalResourceClassPage;
...
public class BareMetalResourceClassEnUSGenApiServiceImpl extends BaseApiServiceImpl implements BareMetalResourceClassEnUSGenApiService {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        siteRequest.setLang("enUS");
              searchBareMetalResourceClassList(siteRequest, false, true, false, "GET").onSuccess(listBareMetalResourceClass -> {
  ...
  public void patchBareMetalResourceClass(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", BareMetalResourceClass.CLASS_AUTH_RESOURCE, "SuperAdmin"));
        if(name != null)
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            JsonArray scopes = authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(o -> "BAREMETALRESOURCECLASS".equals(o.getString("rsname"))).findFirst().map(decision -> ((JsonObject)decision).getJsonArray("scopes")).orElse(new JsonArray());
            if(authorizationDecisionResponse.failed() || !scopes.contains("PATCH")) {
              ...
              searchBareMetalResourceClassList(siteRequest, false, true, true, "PATCH").onSuccess(listBareMetalResourceClass -> {
                try {
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("DELETE")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchBareMetalResourceClassList(siteRequest, false, true, true, "DELETE").onSuccess(listBareMetalResourceClass -> {
                try {
                  ApiRequest apiRequest = new ApiRequest();
                  apiRequest.setRows(listBareMetalResourceClass.getRequest().getRows());
                  apiRequest.setNumFound(listBareMetalResourceClass.getResponse().getResponse().getNumFound());
  ...
  public void searchpageBareMetalResourceClassPageInit(JsonObject ctx, BareMetalResourceClassPage page, SearchList<BareMetalResourceClass> listBareMetalResourceClass, Promise<Void> promise) {
    ...
    ctx.put("enUSUrlSearchPage", String.format("%s%s", siteBaseUrl, "/en-us/search/bare-metal-resource-class"));
    ctx.put("enUSUrlPage", String.format("%s%s", siteBaseUrl, "/en-us/search/bare-metal-resource-class"));
  ...
  public void templateSearchPageBareMetalResourceClass(JsonObject ctx, BareMetalResourceClassPage page, SearchList<BareMetalResourceClass> listBareMetalResourceClass, Promise<String> promise) {
    try {
      ...
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/search/bare-metal-resource-class/BareMetalResourceClassSearchPage.htm"), Charset.forName("UTF-8"));
  ...
  public void templateEditPageBareMetalResourceClass(JsonObject ctx, BareMetalResourceClassPage page, SearchList<BareMetalResourceClass> listBareMetalResourceClass, Promise<String> promise) {
    try {
      ...
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/edit/bare-metal-resource-class/BareMetalResourceClassEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
  ...
  public Future<SearchList<BareMetalResourceClass>> searchBareMetalResourceClassList(SiteRequest siteRequest, Boolean populate, Boolean store, Boolean modify, String scope) {

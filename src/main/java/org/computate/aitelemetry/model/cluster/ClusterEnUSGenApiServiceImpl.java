...
import org.computate.aitelemetry.model.cluster.ClusterPage;
...
public class ClusterEnUSGenApiServiceImpl extends BaseApiServiceImpl implements ClusterEnUSGenApiService {
  ...
  public void searchCluster(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", Cluster.CLASS_AUTH_RESOURCE, "Admin"));
        form.add("permission", String.format("%s#%s", Cluster.CLASS_AUTH_RESOURCE, "SuperAdmin"));
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            JsonArray scopes = authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(o -> "CLUSTER".equals(o.getString("rsname"))).findFirst().map(decision -> ((JsonObject)decision).getJsonArray("scopes")).orElse(new JsonArray());
            ...
            }
            {
              ...
              searchClusterList(siteRequest, false, true, false, "GET").onSuccess(listCluster -> {
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("PATCH")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchClusterList(siteRequest, false, true, true, "PATCH").onSuccess(listCluster -> {
                try {
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(authorizationDecisionResponse.failed() || !scopes.contains("DELETE")) {
              ...
              siteRequest.setScopes(scopes.stream().map(o -> o.toString()).collect(Collectors.toList()));
              List<String> scopes2 = siteRequest.getScopes();
              searchClusterList(siteRequest, false, true, true, "DELETE").onSuccess(listCluster -> {
                try {
                  ApiRequest apiRequest = new ApiRequest();
                  apiRequest.setRows(listCluster.getRequest().getRows());
                  apiRequest.setNumFound(listCluster.getResponse().getResponse().getNumFound());
  ...
  public void searchpageClusterPageInit(JsonObject ctx, ClusterPage page, SearchList<Cluster> listCluster, Promise<Void> promise) {
    ...
    ctx.put("enUSUrlSearchPage", String.format("%s%s", siteBaseUrl, "/en-us/search/cluster"));
    ctx.put("enUSUrlPage", String.format("%s%s", siteBaseUrl, "/en-us/search/cluster"));
  ...
  public void templateSearchPageCluster(JsonObject ctx, ClusterPage page, SearchList<Cluster> listCluster, Promise<String> promise) {
    try {
      ...
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/search/cluster/ClusterSearchPage.htm"), Charset.forName("UTF-8"));
  ...
  public void templateEditPageCluster(JsonObject ctx, ClusterPage page, SearchList<Cluster> listCluster, Promise<String> promise) {
    try {
      ...
      Path resourceTemplatePath = Path.of(siteTemplatePath, pageTemplateUri);
      if(result == null || !Files.exists(resourceTemplatePath)) {
        String template = Files.readString(Path.of(siteTemplatePath, "en-us/edit/cluster/ClusterEditPage.htm"), Charset.forName("UTF-8"));
        String renderedTemplate = jinjava.render(template, ctx.getMap());
        promise.complete(renderedTemplate);
      } else if(pageTemplateUri.endsWith(".md")) {
        String template = siteTemplatePath == null ? Resources.toString(Resources.getResource(resourceTemplatePath.toString()), StandardCharsets.UTF_8) : Files.readString(resourceTemplatePath, Charset.forName("UTF-8"));
  ...
  public Future<SearchList<Cluster>> searchClusterList(SiteRequest siteRequest, Boolean populate, Boolean store, Boolean modify, String scope) {

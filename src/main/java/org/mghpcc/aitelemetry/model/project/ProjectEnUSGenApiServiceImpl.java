...
public class ProjectEnUSGenApiServiceImpl extends BaseApiServiceImpl implements ProjectEnUSGenApiService {
  ...
  public void userpageProject(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
    ...
    user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
      try {
        ...
        form.add("permission", String.format("%s#%s", Project.CLASS_AUTH_RESOURCE, "GETManager"));
        ...
        if(projectResource != null)
          form.add("permission", String.format("%s#%s", projectResource, "GETManager"));
        groups.stream().map(group -> {
              Matcher mPermission = Pattern.compile("^/(.*-?TENANT-([a-z0-9\\-]+))-(GETManager)$").matcher(group);
        ...
        groups.stream().map(group -> {
              Matcher mPermission = Pattern.compile("^/(.*-?HUB-([a-z0-9\\-]+))-(GETManager)$").matcher(group);
              return mPermission.find() ? mPermission : null;
            }).filter(v -> v != null).forEach(mPermission -> {
              form.add("permission", String.format("%s#%s", mPermission.group(1), mPermission.group(3)));
            });
        groups.stream().map(group -> {
              Matcher mPermission = Pattern.compile("^/(.*-?CLUSTER-([a-z0-9\\-]+))-(GETManager)$").matcher(group);
              return mPermission.find() ? mPermission : null;
            }).filter(v -> v != null).forEach(mPermission -> {
              form.add("permission", String.format("%s#%s", mPermission.group(1), mPermission.group(3)));
            });
        groups.stream().map(group -> {
              Matcher mPermission = Pattern.compile("^/(.*-?PROJECT-([a-z0-9\\-]+))-(GETManager)$").matcher(group);
              return mPermission.find() ? mPermission : null;
            }).filter(v -> v != null).forEach(mPermission -> {
              form.add("permission", String.format("%s#%s", mPermission.group(1), mPermission.group(3)));
            });
        ...
        .onComplete(authorizationDecisionResponse -> {
          try {
            ...
            if(!scopes.contains("GETManager") && !classPublicRead) {
              ...
              authorizationDecisionBody.stream().map(o -> (JsonObject)o).filter(permission -> {
                    ...
                    return permission.getJsonArray("scopes").contains("GETManager")
                        && mPermission.find();
                  }).forEach(permission -> {
            ...
            }
            {
              ...
              searchProjectList(siteRequest, false, true, false, "GETManager").onSuccess(listProject -> {
  ...
  public String templateUriUserPageProject(ServiceRequest serviceRequest, Project result) {
    return Optional.ofNullable(result).map(r -> r.getStatusPageTemplateUri()).orElse(String.format("%s.htm", StringUtils.substringBefore(serviceRequest.getExtra().getString("uri").substring(1), "?")));
  }
